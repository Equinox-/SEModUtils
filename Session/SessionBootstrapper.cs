...
{
    public class SessionBootstrapper : ModSessionComponent
    {
        public const ushort MessageChannel = 57655;
        private readonly Dictionary<int, ModSessionComponent> m_hashCodeToComponent = new Dictionary<int, ModSessionComponent>();
        ...
        private void OnComponentAttached(ModSessionComponent k)
        {
        ...
        {
            var stream = MemoryStream.CreateReaderFor(packet);
                ...
                case PacketFullConfig:
                    if (!MyAPIGateway.Session.IsDecider())
                    {
                        ...
                        using (Manager.FallbackLogger.IndentUsing())
                        {
                            ...
                            {
                                ModSessionComponent com;
                                Ob_ModSessionComponent ob;
        ...
        private byte[] CreateFullConfig()
        {
            ...
            using (Manager.FallbackLogger.IndentUsing())
            {
                var stream = MemoryStream.CreateEmptyStream(4096);
        ...
        public override void LoadConfiguration(Ob_ModSessionComponent config)
        {
            ...
            if (config is Ob_SessionBootstrapper) return;
        ...
        {
            return new Ob_SessionBootstrapper();
        }
    }

    public class Ob_SessionBootstrapper : Ob_ModSessionComponent
    {
