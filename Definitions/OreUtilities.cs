...
namespace Equinox.Utils.Definitions
{
    public static class OreUtilities
    {
        ...
        private static List<RarityInfo> m_rarityList = null;
        private static Dictionary<MyDefinitionId, RarityInfo> m_oreRarity = null;
        ...
        public static double GetOutputRatio(MyDefinitionId oreOrIngot)
        {
            ...
            if (oreOrIngot.TypeId == typeof(MyObjectBuilder_Ore))
                return m_avgOutputRatio[oreOrIngot] = BlueprintIndex.Instance.GetAllConsuming(oreOrIngot).Select(x => x.Ingredients.Values.Sum(y => (double)y)).DefaultIfEmpty(1).Average();
            if (oreOrIngot.TypeId == typeof(MyObjectBuilder_Ingot))
                return m_avgOutputRatio[oreOrIngot] = BlueprintIndex.Instance.GetAllProducing(oreOrIngot, true).Select(x => x.Ingredients.Values.Sum(y => (double) y)).DefaultIfEmpty(1).Average();
        ...
        public static double GetRarity(MyDefinitionId ore)
        {
            ...
            if (m_oreRarity == null)
            {
                ...
                foreach (var pobj in lst)
                {
                    ...
                    temp.Add(new RarityInfo() { Id = pobj.Id, OutputRatio = avgOutputRatio });
                }
                // Sort in descending.
                temp.Sort((a, b) => Math.Sign(b.OutputRatio - a.OutputRatio));
                var dout = new Dictionary<MyDefinitionId, RarityInfo>(MyDefinitionId.Comparer);
            ...
            var result = m_oreRarity[ore] = new RarityInfo() { Id = ore, OutputRatio = outRatio, Rarity = rarity };
